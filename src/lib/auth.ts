...
export async function getCurrentUser(details = false): Promise<User | null> {
  ...
  return sessionData?.user ? { ...sessionData?.user, admin: sessionData?.user.admin == "true", lastSeen: sessionData.expiresAt, maps: sessionData.user.maps && sessionData.user.maps.map(m => ({...m, imageGeojsonAvailable: m.imageGeojsonAvailable == "true",
